...
int main(int argc,char* argv[]){
    ...
    int count = 0;  // Count will be used to skip through the first 5 lines of OCC.csv file
    ...
    while (getline(fileStream, line)){ // While there is input from the file, the current line is saved in line variable
        ...
        if(skip == true)
            cout << "";
        else if(line[0] == 'C' && line[1] == 'h' && skip == false){
    ...
    // Creating hash table
    int size = OCC_LEN;
    struct hash_table_entry ** hashTable = new struct hash_table_entry * [findSize(size)];
    for(int i = 0; i < size; i++){  // Initialize the hashtable to null
        hashTable[i] = nullptr;
    }


    // cout << size << endl;    Used the see the size of hash table 
    ...
    for(int i = 0; i<qNum; i++){    // for loop iterates through every query
        ...
        // This section handles the ratio Query, Here is my use of modular design!
        //--------------------------------------------------------------
        int year1 = 0;
        int year2 = 0;
        string ratio = "";
        ...
        // This section handles the hash table Query
        //--------------------------------------------------------------
        ...
        // This section handles the BST Query
        //--------------------------------------------------------------
        else if(queries[i][5] == ' '){
            cout << "Query: " << temp << endl;
        }
            ...
            if(temp[9] == 't'){     // If the 10th char is a t, its asking for total
                ...
                int stop = 0;
                while(soc[stop].total != '\0'){
                    stop++;
                }
                // cout << soc[502].total << endl;
                // cout << "Size of the max-heap is: " << stop << endl;
